<template>
  <div class="component_container object">
    <div class="forms">
      <main>
        <div class="tittle q-mb-lg">Редактировать ОПУ</div>
...
import {formatUnixDateTime, getEnumOptions, toUnixFormat} from "src/lib/helpers";
...
export default defineComponent({
  name: "EditOpuObject",
  components: { SelectMultiple, TextInput, SwitchButton, DateInput },
  directives: { maska },
  setup() {
    ...
    async function loadData() {
      ...
      await store.dispatch("object/getSingleObjectOpu", route.params.item);
      let data = store.getters["object/getOpuObject"];
      if (data) {
        for (let key in data) {
    ...
    async function save() {
      ...
      try {
        ...
        await store.dispatch("object/UpdateObjectOpu", sentForm);
