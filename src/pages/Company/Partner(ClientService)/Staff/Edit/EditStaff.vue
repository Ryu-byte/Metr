...
export default defineComponent({
    ...
    setup() {
      ...
      let form = reactive(cloneDeep(formData));
      ...
      async function loadData() {
        try {
          ...
          await store.dispatch("company/getSingleStaff", route.params.item);
          let data = store.getters["company/getStaff"]
          if (data) {
            for (let key in data) {
              form[key] = data[key]
            }
      ...
      async function send() {
        ...
        try {
          ...
          await store.dispatch("company/UpdateStaff", sentForm);
