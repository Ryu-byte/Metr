    ...
    const rules = computed(() => {
      return {
        ...
        kpp: {
          ...
          integer: helpers.withMessage("Неверный формат КПП", integer),
        },
        password: {
          required: helpers.withMessage(
              "Поле обязательное для заполнения",
    ...
    let submit = async () => {
      ...
      try {
        ...
        $q.notify({
          ...
          type: "positive",
          message:
              "Подтвердите ваш аккаунт по ссылке в письме, отправленном на вашу почту",
        });
      } catch (e) {
        let message = null;
        message = e.message;
    ...
    watch(
        ...
        (value) => {
          if (value) {
            ...
            form.company_full_name = value.data.name.full_with_opf;
            form.company_name = value.data.name.short_with_opf;
            form.legal_address = value.data.address.value;
          }
        },
        {deep: true}
    );
